...
+ [ГЛАВА 2. ИСПОЛЬЗУЕМЫЕ МОДУЛИ](#глава-2-используемые-модули)
  ...
  + [2.2 JavaScript и Canvas](#22-javascript-и-canvas)
...
+ [ГЛАВА 3. ОПИСАНИЕ ПРОЦЕССА МОДЕЛИРОВАНИЯ](#глава-3-описание-процесса-моделирования)
  +[3.1 Связь моделирования и симуляции. Имитация датчика температуры](#31-cвязь-моделирования-и-симуляции-имитация-датчика-температуры)
...
Большая часть данной работы посвящена визуальному моделированию, которое может быть полезно в обучении и просто для лучшего понимания физических процессов. Однако как ужке говорилось ранее, это не единственный вид моделирования, помимо прочего есть также математическое моделирование, которое позволяет нам правильно описывать физические процессы для того, чтобы  в дальнейшем использоать в различных исследованиях или в том же визуальном моделировании объектов.

Симуляция поведения различных объектов, утсройств может быть также использована для настройки, отладки и тестирования программы, ее поведения в тех или иных случаях. Рассмотрим управляющую программу контроллер. Но сперва дадим опредления понятиям:
...
```JS
function init() {
  particles = new Array(); // создание пустого массива частиц

  for(var i = 0; i < numParticles; i++){
  var radius = 7; // задание радиуса

  var speed_x = getRandomArbitrary(0, 10);
  // задание случайной в диапазоне от 0 до 10 скорости по x
  var speed_y = getRandomArbitrary(0, 10); // задание скорости по y

  if (Math.random() > 0.5) speed_x = (-1)*speed_x;
  if (Math.random() > 0.5) speed_y = (-1)*speed_y;

  var mass = 0.01*Math.pow(radius,3); // считаем массу частицы
  ...
  particle.draw(context); // отрисовываем частицу
  }
  walls = new Array(); // массив со стенками сосуда
  var wall1 = new Wall(new Vector2D(canvas.width,0),new Vector2D(0,0));
  // создаем объект Wall с координатами позиции
    wall1.draw(context_bg); // отрисовываем объект
    walls.push(wall1); // добавляем в массив
  ...
  walls.push(wall4);
  t0 = new Date().getTime();
  animFrame(); // функция для анимации
};
```
...
3. Ramtal, D. Physics for JavaScript Games, Animation, and Simulations / D. Ramtal, A. Dobre. $-$ 1st Ed. $-$ 2014. $-$ 467 p.
4. JavaScript documentation [Electronic resource]. $-$ Mode of access: [https://developer.mozilla.org/en-US/docs/Web/JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript). Date of access: 12.01.2023.
5. Canvas documentation [Electronic resource]. $-$ Mode of access: [https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial](ttps://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial). Date of access: 12.01.2023.
6. Python documentation [Electronic resource]. $-$ Mode of access: [https://docs.python.org/3/](https://docs.python.org/3/). $-$ Date of access: 03.03.2023.
7. Pygame documentation [Electronic resource]. $-$ Mode of access: [https://www.pygame.org/](https://www.pygame.org/). $-$ Date of access: 03.03.2023.
8. Pymunk documentation [Electronic resource]. $-$ Mode of access: [http://www.pymunk.org/](http://www.pymunk.org/). $-$ Date of access: 03.03.2023.
9. Matplotlib documentation [Electronic resource]. $-$ Mode of access: [https://matplotlib.org/](https://matplotlib.org/). $-$ Date of access: 03.03.2023.
10. Numpy documentation [Electronic resource]. $-$ Mode of access: [https://numpy.org/](https://numpy.org/). $-$ Date of access: 03.03.2023.
11. The Maxwell–Boltzmann distribution in two dimensions [Electronic resource]. $-$ Mode of access: [https://scipython.com/blog/the-maxwellboltzmann-distribution-in-two-dimensions/](https://scipython.com/blog/the-maxwellboltzmann-distribution-in-two-dimensions/). $-$ Date of access: 23.03.2023.
