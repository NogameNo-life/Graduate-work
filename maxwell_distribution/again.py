...
class Simulation:
    ...
    def __init__(self, n, radius=0.01, styles=None):
        ...
        for item in range(7):
            ...
            self.vel.append((vx,vy))

        self.init_particles(n, radius, styles)
        self.speeds = self.get_speeds(self.get_vel()) """

        self.speed_hist = Histogram(self.data, mu,50, density=True)

    def get_speeds(self, vel_tuple):
        v1 = vel_tuple[0]
        v2= vel_tuple[1]
        return v1
    ...
    def handle_collisions(self):
        ...
        for i,j in pairs:
            if self.particles[i].overlaps(self.particles[j]):
                ...
                self.vel.append((self.particles[i],self.particles[j]))
